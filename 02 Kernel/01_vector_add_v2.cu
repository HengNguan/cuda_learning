...
/*
==== Outcome for 1D and 3D Array adding for both CPU and GPU ====
...
CPU average time: 36.533959 milliseconds
GPU 1D average time: 1.158031 milliseconds
GPU 3D average time: 1.210828 milliseconds
Speedup (CPU vs GPU 1D): 31.548352x
Speedup (CPU vs GPU 3D): 30.172699x
Speedup (GPU 1D vs GPU 3D): 0.956395x
================================================================
Note that, the 3D and 1D implementations have similar performance, with the 3D version being slightly slower.
This is because the 3D implementation has more complex memory access patterns and 
additional overhead for managing the 3D grid and block structure.
Line 39 
*/